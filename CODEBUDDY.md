...
| 特性 | 说明 |
|------|------|
| 宏调用 | ✅ 已实现：`macro_rules!` 声明式宏（`$x:expr`/`$x:ident`/`$x:ty`/`$x:tt` 元变量 + `$(`...`)` 重复 `*`/`+`/`?`，parse 期递归展开为 AST）——`$x:expr` 捕获**完整表达式 token 序列**（token 级优先级爬升：前缀一元 `-`/`!`/`not`/`&`/`*`、二元中缀、后缀调用/索引/成员/`?`/`as` 转换、嵌套宏调用 `m!(x)`，按原样展开优先级由调用方负责）+ 内置格式化宏 `println!`/`print!`/`format!`/`dbg!`（`{}` 值占位、`{:?}` 同构、`{{`/`}}` 转义、多参数可变长度；typecheck desugar 为 String 拼接 + 内建打印）+ 集合宏 `arr!`/`vec!`/`map!`（I3 ✅，§3.9：`arr!` → 数组字面量；`vec!`/`map!` → 块表达式 `Vec::with_capacity(n)` + 逐元素 `push`/`insert`，空集合 → `new()`，元素经子 Parser 解析支持嵌套宏调用与完整表达式）；`r#"..."#` 带哈希原始字符串已实现（lexer：任意 `#` 定界、无转义，与 `r#ident` 区分）；`!` 保留 `not` 一元运算符语义。限制：无卫生宏（hygiene） |
