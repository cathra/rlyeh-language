# P001: 词法分析器核心 (zeta-lexer)

> **模块路径**：`crates/zeta-lexer/`  
> **预估工期**：3-5 天  
> **前置依赖**：无（第一个任务）  
> **输出**：可工作的词法分析器，能将 Zeta 源代码转换为 Token 流

---

## 任务描述

实现一个完整的 Zeta 语言词法分析器（Lexer / Tokenizer），支持所有关键字、运算符、字面量和特殊语法元素。
...
```
between ... and ...    （编译期展开为比较链）
in (a, b, c)         （编译期展开为 == 链）
not in (a, b, c)      （编译期展开为 != 链）
9am, 6pm              （时间字面量）
22:00                 （时间字面量）
```
...
- [ ] `between X and Y` → 分别输出 `Between`, `Ident(X)`, `And`, `Ident(Y)`
...
```rust
// crates/zeta-lexer/tests/lexer_test.rs
...
fn test_between_keyword() {
    let mut lexer = Lexer::new("between 0 and 10");
    let tokens = lexer.tokenize().unwrap();
    assert_eq!(tokens[0].token, Token::Between);
    assert_eq!(tokens[1].token, Token::IntLiteral(0));
    assert_eq!(tokens[2].token, Token::And);  // 'and' 在 between 后面是普通关键字
    assert_eq!(tokens[3].token, Token::IntLiteral(10));
}
