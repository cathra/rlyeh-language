...
pub enum TypeError {
    ...
    /// 对不可变 `static` 赋值（M2：仅 `static mut` 可赋值）
    CannotAssignStatic {
        /// 全局变量名
        name: String,
        /// 源码位置
    ...
    /// 在 `unsafe` 块外访问 / 赋值 `static mut`（SH-P0-1 E3 门禁）
    StaticMutOutsideUnsafe {
        /// 全局变量名
        name: String,
        /// 源码位置
        span: Span,
...
fn write_message(f: &mut fmt::Formatter<'_>, loc: &str, err: &TypeError) -> fmt::Result {
    match err {
        ...
        TypeError::CannotAssignStatic { name, .. } => {
            write!(f, "{loc}: error: cannot assign to immutable static `{name}`")
        }
        TypeError::StaticMutOutsideUnsafe { name, .. } => write!(
            f,
            "{loc}: error: use of mutable static `{name}` must be inside an `unsafe` block"
        ),
...
impl TypeError {
    ...
    pub fn help(&self) -> Option<&'static str> {
        match self {
            ...
            TypeError::CannotAssignStatic { .. } => {
                Some("将声明改为 `static mut` 才能对其赋值")
            }
            TypeError::StaticMutOutsideUnsafe { .. } => {
                Some("将访问 `static mut` 的语句包裹在 `unsafe { }` 块内")
            }
