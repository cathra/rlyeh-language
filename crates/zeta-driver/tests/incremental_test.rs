...
const HELLO: &str = r#"fn main() { println("Hello, Zeta!"); }"#;
const HELLO_V2: &str = r#"fn main() { println("Hello, Zeta v2!"); }"#;

/// 独立临时目录。
fn temp_dir(name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("zeta-incr-test-{}-{name}", std::process::id()));
...
fn first_compile_misses_second_hits() {
    ...
    let first = driver.compile_to_llvm("main.zeta", HELLO).unwrap();
    assert!(!first.cache_hit, "首次编译应为 miss");
    assert_eq!(driver.stats().hits, 0);
    assert_eq!(driver.stats().misses, 1);

    let second = driver.compile_to_llvm("main.zeta", HELLO).unwrap();
...
fn source_change_invalidates_cache() {
    ...
    driver.compile_to_llvm("main.zeta", HELLO).unwrap();
    driver.compile_to_llvm("main.zeta", HELLO).unwrap();
    assert_eq!(driver.stats().hits, 1);

    let third = driver.compile_to_llvm("main.zeta", HELLO_V2).unwrap();
    assert!(!third.cache_hit, "源码变化应触发全量重编译");
    assert_eq!(driver.stats().hits, 1);
    assert_eq!(driver.stats().misses, 2);

    // 回到旧版本 → 缓存仍然有效（产物按哈希归档，互不覆盖）
    let back = driver.compile_to_llvm("main.zeta", HELLO).unwrap();
...
fn force_flag_ignores_cache() {
    ...
    let outcome = forced.compile_to_llvm("main.zeta", HELLO).unwrap();
...
fn cache_is_scoped_per_directory() {
    ...
    a.compile_to_llvm("main.zeta", HELLO).unwrap();
    let outcome_b = b.compile_to_llvm("main.zeta", HELLO).unwrap();
...
fn cache_survives_driver_recreation() {
    ...
    }
    {
        // 新驱动（模拟新进程）应命中磁盘缓存
        let mut driver = IncrementalDriver::new(dir.clone());
        let outcome = driver.compile_to_llvm("main.zeta", HELLO).unwrap();
...
fn incremental_run_source_end_to_end() {
    ...
    let (stdout, first) = driver.run_source("main.zeta", HELLO).unwrap();
    assert_eq!(stdout, "Hello, Zeta!\n");
    assert!(!first.cache_hit);

    let (stdout, second) = driver.run_source("main.zeta", HELLO).unwrap();
    assert_eq!(stdout, "Hello, Zeta!\n");
    assert!(second.cache_hit, "二次运行应命中 LLVM IR 缓存");

    let (stdout, third) = driver.run_source("main.zeta", HELLO_V2).unwrap();
    assert_eq!(stdout, "Hello, Zeta v2!\n");
