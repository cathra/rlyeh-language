//! `HashMap<K, V>` for-in 元组模式迭代集成测试（文件入口 API，自动注入 core.zeta）。
//!
...
fn temp_project() -> PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!("zeta-hashmap-for-{}-{seq}", std::process::id()));
...
"#,
    ...
    assert_eq!(out, "19000\n64\n"); // 20 键 → used=16 时 used*2>=32 触发，cap 翻倍至 64
}
