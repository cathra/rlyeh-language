// fs/ops.rl：文件系统自由函数（2026-09-19 由 fs/module.rl 拆出）。
//
// 归属子模块 `fs::ops`，经 fs/module.rl 的 `pub import` 重导出为
// `fs::read_to_string` / `fs::write` 等原全名。
// 实现说明（N3a/N3b/N3c）：
...
// - `r#rename` 为根命名空间显式引用：根 extern 与 fs 的 rename 同名，裸名 `rename(...)`
//   会被模块内优先解析绑定到本函数（递归，返回 Result 与 0 比较触发
