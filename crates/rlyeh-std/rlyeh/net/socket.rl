// net/socket.rl：socket 字节流自由函数（2026-09-19 由 net/module.rl 拆出）。
//
// 归属子模块 `net::socket`，经 `net/module.rl` 的 `pub import` 重导出为
// `net::socketpair_stream` 等原全名。位运算全链路打通（& | ^ << >>）后启用：
...
// WASI 下无 socket API：各函数返回 Err / 空缓冲（禁用文档化，L4a）。
// 底层 extern（socketpair/socket/connect/close/r#send/r#recv/gethostname）
// 声明于 externs 单元。
