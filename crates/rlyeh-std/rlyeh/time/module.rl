...
// 底层时钟 extern 声明于根模块 core.zeta 的 extern 集中区：
// - `clock()`：libc CPU 时钟（POSIX CLOCKS_PER_SEC=1e6，返回微秒）。
// - `__zeta_clock_monotonic()`：墙钟（clock_gettime CLOCK_MONOTONIC，微秒，
//   S2b；driver 注入 define，不支持平台返回 -1）。
// `Instant::now/elapsed` 优先墙钟（睡眠期间推进），-1 时退回 clock()。
// 语言层无常量定义，1e6 以字面量 1000000 使用。
// 目录化（2026-08）：time/module.zeta = 原 time.zeta（模块规模小，保持单文件）。
