//! C ABI 导出层：Rlyeh 语言侧经 `extern fn` 声明调用（链接静态库接入）。
//!
//! # 消息协议（MVP）
//!
//! - 消息 = [`RlyehMsg`]（`kind` 方法索引 + `a/b/c` 三个 i64 参数槽）；
//! - 消息处理回调 = Rlyeh 编译器为每个 actor 生成的
//!   `_rlyeh_actor_<name>_handle(state, kind, a, b, c) -> u64` 函数，
//!   经 `dlsym(RTLD_DEFAULT, 函数名)` 按符号名解析（Rlyeh 侧传函数名字符串，
//!   语言层无需函数指针表达能力）；
//! - 回调返回 `u64::MAX`（即 -1）表示处理出错 → [`ActorError::Panic`] →
//!   supervisor 按策略决策（重启 / 停止 / 升级）；
//! - 其余返回值作为 ask 回复（send 场景经 [`ActorContext::reply`] 静默忽略）；
//! - 状态内存由 Rlyeh 侧分配（`calloc`），运行时仅持有指针不负责释放
//!   （MVP 无析构，进程退出时由 OS 回收）。
//!
//! # 初始化
//!
//! 全局运行时**懒初始化**：首次 `spawn` / `send` / `ask` 时自动创建
//! （2 workers）；用户可先调 [`rlyeh_actor_init`] 指定 worker 数。
...
/// C 消息结构（与 Rlyeh 侧消息槽布局一致：kind = 方法索引，a/b/c = 参数槽）。
#[repr(C)]
#[derive(Debug, Clone, Copy)]
pub struct RlyehMsg {
    /// 方法索引（Rlyeh 编译器按 actor 方法声明顺序编号）。
    pub kind: u64,
    /// 参数槽 0。
    pub a: u64,
    /// 参数槽 1。
...
/// 消息处理回调签名（Rlyeh 编译器生成的 handle 函数）。
pub type RlyehHandler = unsafe extern "C" fn(u64, u64, u64, u64, u64) -> u64;

/// 状态工厂回调签名（supervisor 重启时重建状态对象）。
///
/// Rlyeh 侧 `__state_new` 实际返回 `i8*`（状态对象指针），故统一为指针返回，
/// 与 wasm（i32）及原生（8 字节）ABI 均匹配；值经 `as u64` 落入状态槽。
pub type RlyehFactory = unsafe extern "C" fn() -> *mut c_void;

/// 桥接 Actor 状态：Rlyeh 侧状态槽值（`__state_new` 返回的 u64，存 actor 字段
/// 槽位地址）+ 编译器生成的 handle 函数。
pub struct CallbackActor {
    /// Rlyeh 侧状态槽值（u64，由 `__state_new` 返回；原生目标下即状态内存地址）。
    ///
    /// `pub(crate)`：ask 快速路径需直接读状态槽并同步调用 handler。
    pub(crate) state: u64,
    /// 消息处理回调。
    pub(crate) handler: RlyehHandler,
}

// 状态内存由 Rlyeh 侧分配，运行时对同一 Actor 的消息处理串行化
...
impl ActorState for CallbackActor {
    fn handle_message(
        ...
        // 回调：state 与消息槽均按 u64 传递（Rlyeh 侧 i64 槽语义）。
...
/// 按符号名解析 Rlyeh 生成的 handle / factory 函数。
///
...
unsafe fn resolve_symbol(name: &str) -> Option<*mut c_void> {
    ...
    // WASI 无 dlsym：查 driver 注入的静态符号表 `rlyeh_actor_resolve`（L4b）。
    #[cfg(target_os = "wasi")]
    {
        let name_c = std::ffi::CString::new(name).ok()?;
...
fn report_err(what: &str, e: &ActorError) {
    eprintln!("rlyeh-actor-runtime: {what}: {e}");
}
...
#[no_mangle]
pub unsafe extern "C" fn rlyeh_actor_init(_workers: i64) -> i32 {
...
/// 创建无监督 Actor。
///
/// `handler` 为 Rlyeh 编译器生成的 handle 函数名（C 字符串），
/// `state` 为 Rlyeh 侧 `__state_new` 返回的状态槽值（u64；原生目标下即状态
/// 内存地址）。返回 ActorRef 不透明句柄（0 表示失败：符号未找到或运行时
/// 未初始化）。
#[no_mangle]
pub unsafe extern "C" fn rlyeh_actor_spawn(handler: *const c_char, state: u64) -> u64 {
    let name = cstr(handler);
    let Some(ptr) = (unsafe { resolve_symbol(&name) }) else {
        eprintln!("rlyeh-actor-runtime: 未找到 handle 符号: {name}");
        return 0;
    };
    let handler: RlyehHandler = std::mem::transmute(ptr);
    #[cfg(target_os = "wasi")]
    {
...
///
/// `handler` / `factory` 为 Rlyeh 编译器生成的 handle / factory 函数名
/// （C 字符串）。`strategy`：0 = OneForOne，1 = AllForOne，2 = RestartForOne。
/// 返回 ActorRef 不透明句柄（0 表示失败）。
#[no_mangle]
pub unsafe extern "C" fn rlyeh_actor_spawn_supervised(
    ...
    let Some(hp) = (unsafe { resolve_symbol(&handler_name) }) else {
        eprintln!("rlyeh-actor-runtime: 未找到 handle 符号: {handler_name}");
        return 0;
    };
    let Some(fp) = (unsafe { resolve_symbol(&factory_name) }) else {
        eprintln!("rlyeh-actor-runtime: 未找到 factory 符号: {factory_name}");
        return 0;
    };
    let handler: RlyehHandler = std::mem::transmute(hp);
    let factory: RlyehFactory = std::mem::transmute(fp);
    #[cfg(target_os = "wasi")]
    {
...
/// （actor 已停止 / 邮箱满 / 运行时关闭）。
#[no_mangle]
pub unsafe extern "C" fn rlyeh_actor_send(
    actor: u64,
    kind: u64,
    a: u64,
    b: u64,
    ...
    #[cfg(target_os = "wasi")]
    {
        ...
        return match crate::sync::dispatch(actor, kind, a, b, c) {
            Some(_) => 0,
            None => {
                eprintln!("rlyeh-actor-runtime: send 失败: actor 已停止");
                1
            }
        };
    }
    #[cfg(not(target_os = "wasi"))]
    {
        ...
        let msg = Box::new(RlyehMsg { kind, a, b, c });
...
#[no_mangle]
pub unsafe extern "C" fn rlyeh_actor_ask(
    actor: u64,
    kind: u64,
    a: u64,
    b: u64,
    ...
    #[cfg(target_os = "wasi")]
    {
        ...
        return match crate::sync::dispatch(actor, kind, a, b, c) {
            Some(reply) => reply,
            None => {
                eprintln!("rlyeh-actor-runtime: ask 失败: actor 已停止");
                0
            }
        };
    }
...
#[no_mangle]
pub unsafe extern "C" fn rlyeh_actor_stop(actor: u64) -> i64 {
    #[cfg(target_os = "wasi")]
    {
...
/// 3 = Stopped，4 = Crashed。
#[no_mangle]
pub unsafe extern "C" fn rlyeh_actor_status(actor: u64) -> i32 {
    #[cfg(target_os = "wasi")]
    {
...
#[no_mangle]
pub unsafe extern "C" fn rlyeh_actor_shutdown() -> i64 {
    #[cfg(target_os = "wasi")]
    {
        crate::sync::shutdown();
        0
