//! # 标准库预置（prelude）加载
//!
//! 编译器自动定位并注入 `rlyeh-std/rlyeh/core.rl`（含其子模块
//! `time.rl` / `io.rl` / `net.rl` / `sync.rl`，经模块展开合入），
//! 使用户程序无需内联即可使用 `Option<T>` / `Result<T, E>` / `Duration` /
//! `read_file` 等核心类型与函数。
//!
//! 定位策略（MVP）：
//! - 环境变量 `RLYEH_STD_PATH` 优先（可指向 `core.rl` 文件或所在目录）；
//! - 否则按源码仓库布局：`<rlyeh-driver>/../rlyeh-std/rlyeh/core.rl`。
...
/// 标准库 `core.rl` 路径：`RLYEH_STD_PATH` 环境变量优先，否则按仓库布局定位。
fn std_core_path() -> PathBuf {
    if let Ok(p) = std::env::var("RLYEH_STD_PATH") {
        let p = PathBuf::from(p);
        if p.is_dir() {
            return p.join("core.rl");
        }
        return p;
    }
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../rlyeh-std/rlyeh/core.rl")
}

/// 加载标准库预置源码（`core.rl` + 子模块展开为单文件）。
///
