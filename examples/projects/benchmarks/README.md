...
6. **`actor_pingpong` 本轮大幅优化（28.3x → 0.55x，341ms → 6.7ms）**：ask 快速路径（fast path）——同线程同步短路：`ask_blocking` 先 `running` CAS 抢占（与 Worker 同一互斥域），抢到后直连 mailbox 检查 + state `try_lock` + CallbackActor superprotocol upcasting 直接 downcast 调 Rlyeh handler，全程零调度/零通道；竞争（running 占用 / 邮箱非空 / 锁被占 / 非 CallbackActor）经 `FastPathOutcome` 原样回退慢路径（`Envelope` 回复通道）。**Rlyeh 7.5ms 超越 Go 11.4ms（1.5x）、C 177ms（23.6x）、Rust/Swift 158–167ms（21–22x），成为全部 6 语言最快**（详见 docs/actor-model.md 附录 A：fast path 纪要）。
