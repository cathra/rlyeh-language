...
fn ok_bytes() -> Result<i64, IoError> {
    let n = write_file(String::from("/tmp/zeta_m3a.txt"), String::from("hello m3a"))?;
    Result::Ok(n)
}

// `?` 传播（失败路径）：读不存在的文件 → Err 上抛
fn fail_read() -> Result<i64, IoError> {
    let content = read_file(String::from("/tmp/zeta_no_such_x.txt"))?;
    Result::Ok(content.len)
}

fn main() {
    // 成功路径：write_file 返回 Ok(字节数)
    let w = write_file(String::from("/tmp/zeta_m3a.txt"), String::from("hello m3a"));
    println(w.is_ok());                    // 1
    match w {
        Ok(n) => println(n),               // 10
        Err(e) => println(-1),
    ...
    // 成功路径：read_file 读回
    let r = read_file(String::from("/tmp/zeta_m3a.txt"));
    println(r.is_ok());                    // 1
    match r {
    ...
    let m = read_file(String::from("/tmp/zeta_no_such_x.txt"));
    println(m.is_err());                   // 1
    match m {
        Ok(v) => println(0),
