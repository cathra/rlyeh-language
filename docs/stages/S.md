...
| S1A | **`Future`/`Poll` protocol 定义**：`enum Poll<T> { Ready(T), Pending }` + `protocol Future { fn poll(&mut self) -> Poll<i64>; }`——关联类型 `type Output` / `Pin<&mut Self>` / `Context` 验证不可行（parser 无 protocol `type` 成员、dyn 不可作函数参数），按计划退化指示 Output 固定 i64；`rlyeh-std/rlyeh/future.rl` + core.rl 重导出 | ✅ 已完成 | [`s1a-future-poll.md`](../tasks/leaf/s1a-future-poll.md) |
