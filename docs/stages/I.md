...
> 现状：声明式宏（含 `$x:expr` 捕获完整表达式 token 序列）、内置格式化宏、集合宏、`r#"..."#` 原始字符串均已实现（I1/I2/I3 ✅）。各任务实现细节、执行情况与技术细节见任务树 [`stage-g-l.md`](../tasks/stage-g-l.md) 对应叶子文档（`i1`–`i3`）。限制：无卫生宏（hygiene）；`Display`/`Debug` trait 定义仍为规划 API（std-lib.md §8）。
...
| I2 | **`Display`/`Debug` trait + 格式化宏**：`{}` 占位符格式化引擎；`println!`/`print!`/`format!`/`dbg!` 宏；String 拼接语义（`{}` 插入 `to_string`） | ✅ | [`i2-format-macro.md`](../tasks/leaf/i2-format-macro.md) |
