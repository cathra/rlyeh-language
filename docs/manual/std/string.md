...
>
> **常见陷阱**：
> - `s[i]` 返回的是**第 i 个字节**（步长 1 字节，ASCII 假设），不是第 i 个字符——处理多字节 UTF-8 字符要用 `chars()` 遍历。
> - `String + String` 是**深拷贝**拼接（返回新缓冲），频繁拼接注意开销；大量拼接可用 `push_str` 原地追加。
> - `s[lo..<hi]` 子串返回**全新 `String`**（值拷贝），和 `&str` 视图不同。
