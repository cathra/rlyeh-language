...
> **C 程序员对照**：C 里堆内存全靠你 `malloc`/`free`，常见的三种隐患是——① 忘记 `free` 泄漏；② 多次 `free` 双重释放；③ 还有指针指向已释放内存（悬垂）。Rlyeh 用这四种智能指针把"何时释放"交给编译器/运行时：
> | 类型 | 对应 C 习惯 | Rlyeh 如何保证安全 |
> |------|------------|---------------------|
> | `Box<T>` | `T* p = malloc(...)` 独占 | 离开作用域自动 `free`（唯一 owner） |
> | `Rc<T>` | 手写 `struct { int refcount; T data; }` | 最后一个引用消失自动 `free`，不会漏/重释放 |
> | `Arc<T>` | 同上 + 原子计数 | 跨线程共享，计数原子操作 |
> | `Gc<T>` | 保守 GC | 循环引用场景用，局部开启 |
