...
> **C 程序员对照**：`Vec<T>` ≈ 你手写的 `struct { T* data; size_t len; size_t cap; }` + `realloc` 增长 + 析构释放，但**全自动**——你不再手写 `realloc`、不再担心忘记 `free`。和 C 数组最大的区别：长度运行时可变、存在堆上、离开作用域自动释放。
>
> **常见陷阱**：
> - 用 `Vec` 前变量须 `let mut`（要 `push` 等修改）。
> - `v[lo..<hi]` 动态切片返回**全新 `Vec`**（值拷贝），和 `&[T]` 零拷贝视图不同（见 [指南 §6.3](../guide/06-arrays-slices.md)）。
> - 索引越界运行时会安全处理（返回 `None` / clamp），不会像 C 那样未定义行为。
