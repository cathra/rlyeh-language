...
| **0.2.0-L** | 诊断信息质量对齐 | 新增 | [SH-P2-6](tasks/leaf/sh-p2-6-diagnostics.md) | 🟠 中 | 🟡 进行中 | L0 harness 诊断维度 + 探针基线落地；L1 typecheck 用户态 span 已对齐（12 例基线 check 12/0/0/0）；borrowck/regionck Span 传播 + L2 结构化诊断待办 |
...
| **0.2.0-Y** | `Send`/`Sync` 自动 trait（放宽/标记） | P3-1 | [SH-P3-1](tasks/leaf/sh-p3-1-send-sync.md) | 🟠 中 | 🔧 M1/M2/M3 已落地（告警式） | **复审补遗**：并发安全基线（告警式）；`Thread::start` 边界 W002 告警 + `is_send_sync` 内建谓词 |
