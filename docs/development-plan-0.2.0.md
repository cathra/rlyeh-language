...
| **0.2.0-M** | 前端自举 PoC | 新增(扩) | [SH-P2-7](tasks/leaf/sh-p2-7-driver.md) | 🔴 高 | 🟡 进行中 | M-M1a 切片1 落地：Rlyeh 版 lexer `self-host/lexer.rl` + 差分对拍 harness（`--emit tokens` oracle）token 逐行一致；M-M1b/c、M-M2/M-M3/M-M4 待推进 |
