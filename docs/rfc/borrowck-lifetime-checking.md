...
| 字段 | 内容 |
|------|------|
| 状态 | 已收口（首轮 T-0~T-5 + §9.8 已落地 2026-09-19；T-6 块级借用经原型评估后启动独立阶段，见 §9.9/§11） |
| 日期 | 2026-09-19 |
...
首轮切片（T-0 → T-5 + §9.8）已全部落地、提交、推送，全量 740+ 用例零回归。

### 10.1 交付物

| 任务 | 内容 | 提交 |
|------|------|------|
| T-0 | `Type::Ref` 携带可选生命周期名（解析→类型贯通） | `98871ed` |
| T-1 | 生命周期名从 parser 贯通至 `Type::Ref`（HIR/类型落点） | `c0e63d2` |
| T-2 | 引用存活边经拷贝/块值传播，闭合别名逃逸缺口 | `77746ae` |
| T-3 | region 边界悬垂检测（`DanglingReference`） | `d236aff` |
| T-4 | region 推断失败诊断收口（方案 A，不新增诊断） | `249d120` |
| T-5 | region 借用良构性 run-pass 测试 | `249d120` |
| §9.8 | `uses` 按名索引不区分同名遮蔽 → 误报修复 | `6ca0846` |

### 10.2 能力增量

- **生命周期名贯通**：`&'a T` 不再解析即丢弃，为后续 region 约束求解铺设类型层基座。
- **引用有效性求解**：
  - 函数帧逃逸（`return` / 块尾值 / 引用别名，T-2）；
  - region 边界逃逸（区内局部被引用、region 退出后引用仍存活 / 区尾引用逃逸到区外变量，T-3）。
- **同名遮蔽误报修复**（§9.8）：`last_use` 按绑定实例计算，消除跨作用域污染。
- **诊断质量**：错误坐标精确到表达式/语句/块级（取代函数级粗粒度），含 `= help` 与关联位置回指。

### 10.3 遗留与后续

- **T-6（B-7 块级借用）**：首轮明确列为独立后续阶段；经原型验证后于独立阶段启动（见 §9.9 / §11）。
- **非目标未触及**：HIR 字段内引用悬垂（字段/索引链不可追踪）、`&mut`/生命周期不一致显式标注检查、use-after-move（P005 遗留）仍待各自专项。
