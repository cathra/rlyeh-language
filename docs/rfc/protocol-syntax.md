# RFC：Protocol 语法（`protocol` / `impl T: P` 取代 `trait` / `impl Trait for Type`）

| 字段 | 内容 |
|------|------|
| 状态 | Draft（4 项开放问题已于 2026-09-18 决议；PC-0 / PC-1 已落地；**同日二次复议 PC-8：扩展块语法改为 `impl T: P` / `impl T`**（§7.7）；**PC-7 活文档/技能全量改写已落地**（§7.8）；**PC-9 多协议 `impl T: A, B` 已落地**（§7.9）；**PC-10 `dyn A → dyn B` 静态上转已落地**（§7.10）；**PC-11 `trait` / `extension` 关键字已彻底移除**（§7.11）；**PC-12 旧语序 `impl P for T` 已移除、语法收敛为 `protocol` + `impl T: P` / `impl T`**（§7.12）；其余待评审；评审通过后再并入 `grammar.md` / `semantics.md`） |
| 日期 | 2026-09-18 |
...
Rlyeh 的抽象机制沿用了 Rust 的 `trait` + `impl ... for ...` 模型。其**语义内核**（方法集约束、单态化、`dyn` 分派、关联类型、默认方法）是正确且必要的，但**语法形态**带有两个明显的"Rust 味"负担：

1. **`trait` 一词的认知负荷**：`trait` 是 Rust 特有术语（源自 Scala），在主流语言（Swift `protocol` / Java·C# `interface` / Go `interface` / C++ `concept`）中，同类概念几乎都叫"协议/接口"。用户读到 `trait` 时会不自觉代入 Rust 的所有权/`dyn`/对象安全等一整套语境。

2. **`impl <Trait> for <Type>` 违反主流"声明点一致性"习惯**：
   ...
   - **固有方法与协议实现视觉难区分**：`impl Foo { }`（固有）与 `impl Trait for Foo { }`（协议）只差一个 `for` 子句，阅读/搜索时极易混淆。

**核心主张**：用 Swift 风格的 `protocol` + **声明点一致性**（`struct C: P`）替换 `trait` + `impl P for C`；用 **`impl T: P` / `impl T`**（Swift 式语序，`impl` 关键字保留）承担**追溯式一致性**与**固有方法**。Rlyeh 现有的 `:` 已经承载"约束/归属"语义（`T: Bound`、字段 `name: Type`、`where T: Bound`），把 `C: P` 复用为"类型 C 归属于协议 P"，**心智一致、零新增符号**。
...
- **G1**：`protocol` 成为声明"行为契约"的唯一关键字；`trait` 关键字已移除（PC-11，见 §7.11）。
...
- `protocol` 为**保留字**；原 `trait` 关键字**已从语法中移除**（PC-11，见 §7.11）。
- 语法位置与 `trait` 完全一致：`pub`? 可修饰、泛型参数、`where`、`:`（协议继承）、成员体。
...
- **`extension` / `trait` 关键字已移除**：语法只接受 `protocol`（协议声明）与 `impl T: P` / `impl T`
  （一致性 / 固有实现）。二者已不再是保留字（PC-11，见 §7.11）。

### 3.5 协议继承：`protocol A: B`

> **现状**：`grammar.md` §2.6 已写 `TraitDecl ::= ... ':' TraitBound? ...`，但 `parse_trait` **从未实现** `:` 处理，全仓无 `trait A: B` 用例——即**规划未落地**。本方案顺带以 Swift 同形语法落地。
...
语义：`protocol A: B` 表示 **A 的见证者必须同时是 B 的见证者**（`A` 蕴含 `B`）；对 `dyn A` 可安全上转 `dyn B`。约束检查复用 `type_implements_trait` 的传递闭包。

**已落地（PC-4 / PC-10，2026-09-18）**：

- 一致性义务校验（实现 `A` 必须实现 `B`，否则 `TC029`）+ `type_implements_trait` 传递闭包（PC-4）；
- **vtable 线性化**：`dyn T` 的方法槽按「supertrait 方法在前、本协议方法在后（同名去重）」排列，
...
```rlyeh
let shapes: Vec<dyn Draw> = ...;         // trait object → dyn 协议
...
| # | 现状（trait / impl） | 方案（protocol / impl） |
|---|----------------------|------------------------------|
| 1 | `trait Area { fn area(&self) -> f64; }` | `protocol Area { fn area(&self) -> f64; }` |
...
| 8 | （supertrait 规划未实现） | `protocol Loud: Named { .. }`（**新增**） |
...
- 语法仅接受 `protocol` 声明与 `impl [<G>] Type (: ProtocolList)?` 一致性 / 固有实现；`trait` / `extension`（PC-11）与旧语序 `impl P for T`（PC-12）均已移除，**AST 不新增顶层节点类型**（仅扩充既有 `AstTraitDecl` / `AstImplBlock` 字段 + `AstStructDecl` 增加一致性/成员字段）。

---

## 6. 语义映射（零成本 desugar）

本方案是**纯语法重排**，`desugar` 阶段把新写法归一到既有的 `AstTraitDecl` / `AstImplBlock` 结构，**typecheck / codegen 零改动**：

| 新写法 | desugar 归一结果 |
|--------|------------------|
| `protocol P { .. }` | `AstTraitDecl { .. }`（原样，仅关键字替换） |
| `protocol A: B { .. }` | `AstTraitDecl` 增加 `supertraits: Vec<..>`；typecheck 校验 `A` 见证者含 `B` |
| `struct C: P { fields; members }` | `AstStructDecl { fields, conformances: [P] }` + 内联成员按 §3.3 裁决拆为 `AstImplBlock{ trait_name: Some(P), type_name: C }` 与 `AstImplBlock{ trait_name: None, type_name: C }`（固有） |
| `enum E: P { .. }` | 同上（enum 版本） |
| `impl C: P { .. }` | `AstImplBlock { trait_name: Some(P), type_name: C, .. }` |
| `impl C { .. }` | `AstImplBlock { trait_name: None, type_name: C, .. }` |
| `impl<T> C<T>: P where ..` | `AstImplBlock { generics, trait_name: Some(P), type_name: C, .. }`（where 已由既有 `parse_where_clause` 支持） |
| `impl C: A, B { .. }`（多协议） | `AstImplBlock { trait_name: Some(A), extra_traits: [(B, ..)], .. }` + desugar 按协议成员名裁决拆分为 `impl A for C` / `impl B for C`（PC-9，见 §7.9） |
| `impl<T> C<T> { .. }`（固有泛型） | `AstImplBlock { generics, trait_name: None, type_name: C, .. }` |

因此：

- **单态化缓存、接口哈希（`INTERFACE_VERSION`）、vtable 布局、`dyn` 上转、`Self` 替换、默认方法回退** 全部不变。
- 需要新增的**唯一语义能力**是 §3.5 协议继承（`supertraits` 的传递约束 + `dyn` 上转），其余为新语法糖。
...
1. **阶段 P0（已完成）**：`protocol` 为协议声明关键字；扩展块沿用 `impl`（`impl T: P` / `impl T`）。`trait` / `extension` 关键字已移除（PC-11，见 §7.11），旧语序 `impl P for T` 亦已移除（PC-12，见 §7.12）。除与关键字撞名的标识符需改名外（见 §7.5），迁移后的代码照常编译。
2. **阶段 P1（已完成）**：`rlyeh fmt` 默认即输出新语法（`protocol` / `impl Type: Protocol`）；std / tests / examples / 文档代码块的旧语法已全部迁移（PC-7 / PC-11 / PC-12）。
3. **阶段 P2（弃用告警，已撤销）**：曾对旧写法发 `warning[W002]`；随着 `trait` / `extension`（PC-11）与旧语序 `impl P for T`（PC-12）从语法中移除，该项已无对象，`--deny-deprecated` 与 `--legacy` 选项一并移除。
4. **阶段 P3（移除，已完成）**：`trait` / `extension` 关键字（PC-11）与 `impl P for T` 的 `for` 语序（PC-12）均已移除；语法收敛为 `protocol` + `impl T: P` / `impl T`。

### 7.2 迁移映射（工具依据）

```
trait N { .. }                     →  protocol N { .. }
trait N: S { .. }                  →  protocol N: S { .. }          （S 此前非法，新能力）
...
- 同一类型对同一泛型协议的**多 impl**（`impl Wrap<i64> for W` / `impl Wrap<bool> for W`）——工具需保留为两个 `impl W: Wrap<i64>` / `impl W: Wrap<bool>`（注意 `TraitRef` 泛型实参随行）。
...
>
> **结论**：`protocol` 是唯一**新增**的硬保留字，标识符撞名不可回避。`trait` / `extension`（PC-11）与旧语序 `impl P for T`（PC-12）均已从语法中移除；迁移可由 `rlyeh fmt` 完成，关键字撞名可用「保留字用作标识符」诊断定位。
...
- **语法**：`crates/rlyeh-parser/src/parser.rs` 的顶层项分派 / `pub` 分派 / `is_item_start` 纳入两关键字；`item.rs` 的 `parse_trait` 接受 `trait` **或** `protocol`（归一为 `AstTraitDecl`）；新增 `parse_extension` 支持
...
- **兼容性**：`trait` / `impl` 旧写法**完全兼容**（零回归）；3 处关键字撞名已改名（§7.5）。
...
- **语法**：`crates/rlyeh-parser/src/item.rs` 的 `parse_impl` 支持新语序
  ...
  兼容旧语序 `impl Trait for Type`；抽出 `skip_type_generic_args` 复用泛型实参消费；
  `parse_extension` 保留为弃用兼容入口（单协议，多协议仍显式报错）。
- **诊断**：`rlyeh-typecheck` 的 `deprecated_keyword_warnings` 覆盖 `trait` / `extension` 关键字
  ...
  /「`impl Type for Trait` 旧语序已弃用」。
- **格式化**：`tools/rlyeh-fmt` 默认输出 `impl Type: Trait` / `impl Type`；`--legacy` 输出旧写法。
...
- **标准库**：`docs/std-lib.md`（`trait` → `protocol` 全量；`impl X for Y` → `impl Y: X`）。
...
- **历史过程记录**：`docs/tasks/**`、`docs/stages/**`、`docs/development-plan*.md`、`CHANGELOG.md`
  ——记录当时状态，不追溯改写。
- **实现层错误消息原文**：`CODEBUDDY.md` 中 `` `type X does not implement trait B` ``（与 typecheck
  实际消息一致，待后续统一诊断措辞时一并调整）。
- **关键字兼容产生式**：`grammar.md` 的 `'protocol' | 'trait'` 与 `ImplDecl` 两条过渡兼容分支。
- **Rust 对比语境**：基准报告中「Rust trait 对象」等对 Rust 语言的描述。
...
- **AST**：`AstImplBlock` 新增 `extra_traits: Vec<(String, Vec<AstType>)>`
  （`trait_name` 为列表首个协议，其余入此）。
...
- **验证**：`rlyeh-parser` 新增 `test_impl_multi_conformance_list` /
  ...
  `ast-user` 基线重生成（`AstImplBlock` 新增 `extra_traits` 字段）、5 个快照维度全 0 差异。

### 7.10 PC-10 `dyn` 上转实现纪要（2026-09-18）

**`dyn 子协议 → dyn 父协议` 静态上转**已落地（纯编译期类型转换，**零运行时开销**）：

- **vtable 线性化**（`check_expr::linearize_trait_methods`）：`dyn T` 的方法槽按
  **supertrait 方法在前、本协议方法在后**排列，同名方法去重（保留基类版本），多级继承递归处理。
...
- **上转**（`check_stmt` 的 `dyn_supertrait_upshift` 分支）：目标为 `dyn B`、源为 `dyn A`
  （或 `&dyn A → &dyn B`，引用层同构）且 `A` 的父协议传递闭包含 `B` 时，**复用同一胖指针**、
  仅编译期改类型。
- **兼容性**：无 supertrait 的协议线性化结果 = 其自身方法声明序，**与既有 vtable 布局完全一致**
...
### 7.11 PC-11 移除 `trait` / `extension` 关键字（2026-09-18）

**`trait` 与 `extension` 已从语法中彻底删除**——二者不再是保留字（可作普通标识符），
声明形式不再可解析：

- **词法**：`rlyeh-lexer` 删除 `Token::Trait` / `Token::Extension` 变体与关键字映射。
- **语法**：`parse_trait` 仅接受 `protocol`；`parse_extension` 函数整体删除；`parser.rs` 的
  顶层 / `pub` 分派与 `is_item_start` 相应收窄。
- **诊断**：弃用告警仅保留**旧 impl 语序** `impl P for T` → `impl T: P`（`trait` / `extension`
  已非关键字，无对应告警）。
- **格式化**：`rlyeh fmt` 始终输出 `protocol`；`--legacy` 现仅影响 impl 语序
  （`impl T: P` → `impl P for T`）。
- **工具**：`rlyeh-doc` 的文档分类标签 `Trait` → `Protocol`。
- **迁移**：`tests/**`（compile-pass / compile-fail / run-pass 及 `benches`、驱动集成测试的内联源码）
  的 `trait X` → `protocol X`；std / examples 此前已迁移（PC-7 / PC-8）。
...
- **验证**：`rlyeh-lexer` 单测更新为 `test_protocol_keyword`；`rlyeh-parser` 新增
  `test_trait_extension_are_plain_identifiers`（二者可作变量名）与 `test_protocol_and_impl_keywords`；
  ...
  `ast-user` / `hir-user` 基线重生成（用例源码由 `trait` 改为 `protocol` 致 span 偏移）、
  5 个快照维度全 0 差异。

### 7.12 PC-12 移除旧 impl 语序 `impl P for T`（2026-09-18）

**Rust 式旧语序 `impl Trait for Type` 已从语法中彻底移除**——`impl` 块**唯一语序**为
`impl [<G>] Type (: ProtocolList)? WhereClause? { .. }`：

- **语法**：`parse_impl` 只解析新语序（`first` 即被实现类型）；删除 `for` 分支与
  `looks_like_generic_trait_impl` lookahead 辅助。
...
| 层 | 落点 | 改动 |
|----|------|------|
| 词法 | `crates/rlyeh-lexer/src/{token.rs,lib.rs,tests.rs}` | 新增 `Protocol` 保留字；`Impl` 保留（改语序）；`Trait` / `Extension` 过渡期弃用 |
| 语法 | `crates/rlyeh-parser/src/item.rs` | `parse_trait` 接受 `protocol`（含 `:` 继承）；`parse_impl` 支持 `impl T: P` 语序（兼容 `for` 旧语序）；`parse_extension` 转为弃用兼容；`parse_struct`/`parse_enum` 增加一致性列表与内联成员 |
| AST | `crates/rlyeh-ast/src/lib.rs` | `AstTraitDecl` 加 `supertraits`；`AstStructDecl`/`AstEnumDecl` 加 `conformances` + `members`；旧字段兼容或统一 |
...
| PC-4 | 协议继承 `protocol A: B`（supertraits 传递约束 + `dyn` 上转）（**传递约束已落地**；`dyn` 上转由 PC-10 落地，见 §7.10） | P1 | 中 | PC-1 |
...
| PC-11 | **移除 `trait` / `extension` 关键字**（词法 / 语法 / 诊断 / 格式化 / 工具 / 用例 / 文档）（**已落地**，2026-09-18，见 §7.11） | P1 | 中 | PC-8 / PC-9 |
...
- **已移除**：`trait`（→ `protocol`）、`extension`（→ `impl T: P` / `impl T`）——不再是保留字（PC-11）。
...
|---------------|-----------------|-------|---------|------|
| `trait P` | `protocol P` | `protocol P` | `interface P` | `trait P` |
...
| `trait A: B`（规划未实现） | `protocol A: B` | `protocol A: B` | `interface A extends B` | `trait A: B` |
