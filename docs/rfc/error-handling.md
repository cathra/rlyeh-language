...
| 字段 | 内容 |
|------|------|
| 状态 | 评审通过（Accepted）；已并入 0.2.0 计划（阶段 AA：eh-1…eh-8）。**进度（2026-09-21）**：eh-1 ✅、eh-2 ✅、eh-3 🟠（非闭包 + 函数值型 + 嵌套泛型组合子 ✅；typecheck「fn 形参泛型推断」与 parser/typecheck「嵌套 self 类型」两处缺口已修；仅剩闭包字面量实参与 `copied`/`cloned`）、eh-8 边界硬化（M4）✅ |
| 日期 | 2026-09-20 |
