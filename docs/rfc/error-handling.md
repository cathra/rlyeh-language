...
- `try` 块：`try { ...; Ok(x) }` 内部 `?` 冒泡到块尾 `Result`（对齐 Rust `try`/返回块）。**✅ 已落地（2026-09-21，EH-6 M1）**——见 [`../tasks/leaf/eh-6-try-aggregation.md`](../tasks/leaf/eh-6-try-aggregation.md)；验收 `tests/run-pass/eh_try_block.{rl,out}`，全量 346/346。
...
- **实现分析与阻塞（`try` 块）**：Rlyeh 的 `?` 由 typecheck 在 `check_question` 中**合成 `match … { Err(e) => return Err(e) }` AST**——即**硬编码到函数边界**；`try` 块需要「残差（residual）目标」上下文（该 `?` 改为 `break`/赋值到块级累加器）。**已落地的方案（2026-09-21，EH-6 M1）**：① 新增 `try` 关键字（lexer + parser，已核实 `try` 此前未用作标识符，零迁移成本）；② AST 新增 `TryBlock`/`TryBreak`；③ typecheck 的残留上下文**只需深度计数**（`check_question` 在 `try_loop_bases` 非空时把 `?` 失败臂改 `Return`→`TryBreak`，错误类型仍取 `current_return_type` 的 `E`，`From` 自动转换原样复用，**无需类型推断**、无需块级 `Result` 目标类型）；④ `break <value>` 的 HIR→codegen 通路已在前置提交打通（`loop_break_value` 验收）；⑤ 隐藏函数 + 参数传递方案受阻于**闭包捕获不支持**（H3）——**未采用**，改用 `loop { … break <value> }` 隐式循环 desugar。详见 [`../tasks/leaf/eh-6-try-aggregation.md`](../tasks/leaf/eh-6-try-aggregation.md)。
- **进展（2026-09-21）**：**④ 已解决** —— `break <value>` 通路此前在 typecheck（`ExprKind::Loop` 恒 `Type::Never`）与 MIR（`lower.rs` 显式丢弃值、出口块恒 `Unit`）两处断裂，现补齐 `TypeContext::loop_break_types` 与 `LoopCtx::break_value`，`let x = loop { break 5 };` 得 `5`（验收 `tests/run-pass/loop_break_value.{rl,out}`，全量 345/345）。
  余下设计已探明并记录于 [`../tasks/leaf/eh-6-try-aggregation.md`](../tasks/leaf/eh-6-try-aggregation.md)：**`try` 未在 std / tests 中作标识符**（关键字风险低）；残留上下文只需**深度计数**（错误类型仍取外层函数 `current_return_type`，`From` 转换逻辑原样复用，**无需类型推断**）；desugar 形态 `try { body }` → `loop { <body>; break <Kind>::<OkVariant>(<tail>) }` 的机制已逐一探针验证（含裸 `Result::Ok` 的 `E` 宽松可接受）；另需两处收口——`?` 残留用独立 AST 节点（不污染循环 break 值类型，避免 Ok 位未定）与**拦截 `try` 体内裸 `break` / `continue`**（否则会绑定到隐式循环）。
