...
| SH-P2-8 | `mem::swap` / `mem::replace` 内建 | 0.2.0-U | typecheck / borrowck / desugar / regionck | [leaf](./leaf/sh-p2-8-mem-swap.md) | 🟢 完成（M1 `mem::swap` 三次 memcpy 交换；M2 `mem::replace` desugar 复用 mem::swap 统一处理标量/聚合；M3 `mem::take` 依赖 Default trait 暂缓） |
