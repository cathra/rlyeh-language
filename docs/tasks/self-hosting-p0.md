...
|----|------|-----------|-----------|------|------|
| SH-P0-1 | `unsafe` 块 / 裸指针 / `#[repr(C)]` | 0.2.0-E | gc-runtime / region-alloc / actor-runtime ffi / rlyeh-std nio | [leaf](./leaf/sh-p0-1-unsafe.md) | ⏳ 规划中 |
| SH-P0-2 | 跨函数边界闭包 + `move` + `'static` | 0.2.0-F | actor-runtime / driver 线程模型 | [leaf](./leaf/sh-p0-2-closure.md) | ⏳ 规划中 |
| SH-P0-3 | `dyn Trait` 含 `Self` 方法 + `Any` 类型擦除 | 0.2.0-G | actor 消息协议 | [leaf](./leaf/sh-p0-3-dyn-any.md) | ⏳ 规划中 |
...
P0 全部 ⏳ 规划中（**0.2.0 必须项（语言特性）**，对应阶段 E/F/G/H）。
