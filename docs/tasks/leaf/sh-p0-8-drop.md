...
## 目标
支持 `protocol Drop { fn drop(&mut self); }` + `impl Drop for T`，作用域结束（拥有所有权变量）自动调用 `drop`，实现 RAII：MutexGuard 自动解锁、arena 自动释放、`Rc`/`Arc` 计数递减、`File` 自动关闭。Rlyeh 0.1.0 完全无析构机制（MVP 靠注入解锁/OS 回收，std-lib.md 多处标注「无 Drop」）。

## 现状
- 语言无 `Drop` protocol；codegen 不发射作用域尾清理。
- 受影响 Rust 事实：`sync/module.rl` 目标 `MutexGuard` 作用域尾自动解锁、`rlyeh-std` 多处「进程退出时由 OS 回收（无 Drop）」、`Vec`/`HashMap` 析构。

## 风险分解（→ 中/低危）
- **M1（中）** `protocol Drop { fn drop(&mut self); }` 声明 + `impl Drop for T` 检测（与现有 protocol/impl 机制复用，需 A 泛型 protocol 落地后接）。
...
## 缺口复核（2026-09-02，动手前）
叶子「现状」称「语言无 `Drop` protocol；codegen 不发射作用域尾清理」，复核发现
**四项缺口层次各不相同**，且 Q-M2 的**注入机制其实已存在**：

| 项 | 规划描述 | 复核结果 |
|----|---------|---------|
| **Q-M1** `Drop` protocol + impl 识别 | 完全缺失 | 语言层**确为缺口**（全部 `Drop` 命中都在 Rust 侧运行时/测试）；但已有内置 protocol 先例（`Any`）可同构实现 |
...
另：M1 原注「需 A 泛型 protocol 落地后接」。复核认为**非阻塞**——`Drop` 本身是
非泛型 protocol，具体类型上的 `impl Drop for Foo` 复用既有 protocol/impl 机制即可；
仅 `Vec<T>` 这类泛型类型的 Drop 才依赖 0.2.0-A。

## 实现纪要（2026-09-02）

### Q-M1：`Drop` 作为内置 protocol
`is_drop_protocol`（`check_expr/misc.rs`，与 `is_any_protocol` 同构）：`Drop` 无论是否
显式 `protocol Drop` 声明都生效，解析名可带模块前缀。`has_drop_impl(ctx, ty)` 按
「protocol 名匹配 + `type_matches` 目标类型」在 `ctx.impl_defs` 中查找。
