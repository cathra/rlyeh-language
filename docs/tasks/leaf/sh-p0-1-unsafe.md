...
- ✅ **E2 `#[repr(C)]` 真布局（sub-8 字节标量 C 打包，已实现并验证）**：属性解析 + `repr_c` 标志已下传（`AstStructDecl`），并将真实字段尺寸 / 偏移 / 提升方式经 `FieldScalar::ReprCField { offset, field_ty, conv }` 自 HIR 经 MIR→LIR→codegen 下传（无需改动 250 处 `FieldGet/FieldSet` 构造，仅 ~6 个匹配臂 + 构造点）。
  - **字段访问**（`FieldGet`/`FieldSet`/`FieldAddr`）与**结构体字面量构造**（ctor `FieldSet`）均按 C 规则落位：自然对齐（size 上取整对齐）、紧密打包（a@0, b@4, c@8...），窄字段读经 `zext`/`sext`/`fpext` 提升为宽值、写经 `trunc`/`fptrunc` 降窄；聚合字段（嵌套结构体 / 枚举 / 数组 / 字符串视图 / dyn Protocol）仍由**安全护栏**显式拒绝（`collect.rs`：`c_field_size(&ty).is_none()`）。
