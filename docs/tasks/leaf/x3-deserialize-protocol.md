# X3 `Deserialize` protocol + Serializer/Deserializer 框架

> **所属阶段**：阶段 X
> **状态**：✅ 已完成（`Deserialize` protocol ✅；`JsonError`/`TomlError` ✅；Serializer/Deserializer 访问器框架 ✅；`json::try_parse::<T> -> Result<T, JsonError>` Err 路径 ✅，2026-08-30）
> **依赖**：U4、U3
> **所属任务树**：[任务文档导航](../README.md) → [阶段 U–Z](../stage-u-z.md)

## 目标

`protocol Deserialize { fn from_json(s: String) -> Self; }` + Serializer/Deserializer 框架。
...
`protocol Deserialize { fn from_json(s: String) -> Self; }`（依赖 U4 `-> Self`，替换「解析统一走内建」退化）；`Serializer`/`Deserializer` 访问器框架（目标签名 `serialize(&self, &mut Serializer) -> Result<(), SerError>`，U3 约束泛型化 `to_string<T: Serialize>` 落地）；`JsonError`/`TomlError` 类型；`json::parse` 非法输入改返回 `Err`。

## 实施情况（2026-08-27，核心完成）

- **`Deserialize` protocol + 手写 impl**：`protocol Deserialize { fn from_json(s: String) -> Self; }`，`-> Self` 返回可行（U4）；`impl Point: Deserialize` 手写 + `Point::from_json(...)` 调用（`x3_deserialize_protocol.rl`）。泛型 protocol 约束 `fn f<T: Foo>(x: T)` 验证可行（`to_string<T: Serialize>` 前置）。
- **std serde 模块**：serde/module.rl 定义 `Deserialize` protocol + `enum JsonError { ParseError(String), InvalidType }` + `enum TomlError`；用户经 `serde::JsonError`/`serde::TomlError` 路径访问（`x3_error_types.rl`：`Result<i64, JsonError>` + match 解包）。
...
- [x] 手写 `impl Deserialize` + `from_json` 调用（`x3_deserialize_protocol.rl`）。
...
| 2026-08-27 | Deserialize protocol + JsonError/TomlError 错误类型 + Result 错误处理 |
