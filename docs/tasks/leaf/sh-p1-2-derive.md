...
> **级别**：P1（阻塞前端自举） · **状态**：🟢 核心落地（struct 三 trait） · **归属**：0.2.0-C
...
- **机制**：derive 展开 **零新增 IR 节点**——`parse_attributes` 已收集 `#[derive(...)]` 名列表；
  ...
  为每个受支持 trait 合成一个 `AstImplBlock` 并走既有 `collect_impl`（与手写 `impl` 完全一致），
  方法体在调用点实例化。
- **C2 `Clone`**：`trait Clone { fn clone(&self) -> Self; }`（`core.rl` 顶层新增）；
  derive 生成逐字段拷贝——标量 / 引用直接拷贝，其余（String / 嵌套 struct / 泛型字段）调 `.clone()`。
- **C3 `PartialEq`**：`trait PartialEq { fn eq(&self, other: &Self) -> bool; }`（`core.rl` 顶层新增）；
  derive 生成逐字段 `==` 链；并在 `comparison.rs` 将结构体 `==`/`!=` **desugar 为 `a.eq(&b)` / `!a.eq(&b)`**
  （仅当该类型实现了 `PartialEq`——手写或 derive，否则保留「仅 String 支持内容相等比较」错误）。
- **C1 `Debug`**：复用 `fmt` 模块既有 `trait Debug { fn fmt(&self, f: &mut Formatter) -> Result<(), FmtError>; }`；
  derive 生成 `fmt` 体——标量走 `int_to_string`/`float_to_string`、bool 走字面量、String 走 `write_str`、
  嵌套类型走 `.fmt(f)`；经 `dbg!`（`{:?}` 语义）输出 `Name { f0: <Debug>, ... }`。
- **C4 框架**：`canonical_trait` 映射受支持名（`Clone`/`PartialEq`/`fmt::Debug`），未知名（如 serde
...
`rlyeh-std/rlyeh/core.rl`（`Clone`/`PartialEq` trait）、`rlyeh-std/rlyeh/fmt/module.rl`（`Debug` 既有）。
...
| 2026-09-02 | struct `#[derive(Clone/PartialEq/Debug)]` 核心落地：derive 展开 + `==`/`!=` desugar + std trait；全量回归 252/252 |
