...
`Formatter` 从占位类型升级为真实格式化器（对齐/宽度/精度/填充状态字段）；`protocol Display { fn fmt(&self, f: &mut Formatter) -> Result<(), FmtError>; }`（目标签名，依赖 U3/U4，替换 `-> String` 降级；`Debug::fmt_debug` 改名对齐 `Debug::fmt`）；`FmtError` 类型。
...
- **语言级：impl 方法符号带 protocol 区分**（`Debug::fmt` 同名前提）：`instantiate_impl_method`/`check_method_call` 的 `base_fn` 对 protocol impl 加 protocol 后缀（`Point::fmt`（Display）→ `Point::fmt__fmt::Display`），消除同名 protocol 方法符号碰撞。
- **语言级：方法调用按 protocol 分派**：`ExprKind::MethodCall` 新增 `protocol_hint` 字段；`check_method_call` 支持 `protocol_hint`（`fmt::Display`/`fmt::Debug`）经 `find_impl_for_protocol_method` 精确分派。
...
- [x] `display_fmt.{rl,out}`（`{}`/`{:?}` 按 protocol 区分 + `dbg!` + 内建类型兼容 + `.out` 精确对比）。
- [x] `x4_format_align.{rl,out}`（对齐占位符：右/左/居中/自定义填充 + 宽度不足原样 + `.out` 精确对比）。
- [x] 全量回归：158 用例通过（`()` 值、codegen payload、方法符号带 protocol、MethodCall protocol_hint 改动零破坏）。
...
| 2026-08-27 | Formatter 升级 + FmtError + Display/Debug 签名改 Result<(),FmtError> + Debug 改名 fmt + () 单元值支持 + 方法符号带 protocol 区分 + MethodCall protocol_hint 分派 |
