...
> **所属阶段**：阶段 Y
> **状态**：🔧 部分完成（Y2a kqueue FFI 绑定 ✅，2026-08-28；Y2b Poller 分派待办）
> **依赖**：R
...
### Y2b（中风险，依赖 Y2a）
- `Poller` 内部按 `__rlyeh_target_os()` 分派 kqueue（macOS）vs poll（兜底）vs epoll（Linux）；`register`/`deregister`/`poll` 后端切换。为 W3 事件驱动 executor 提供平台后端。

## 验证

`y2a_kqueue.{rl,out}`（160 用例全绿）；`nio_epoll_test.rs`（Linux）/`nio_kqueue_test.rs`（macOS 或 CI 矩阵，Y2b）。
