...
`toml::to_string(v)` ≡ `toml::stringify(v)`（标量/数组/Vec/嵌套表/HashMap 内联表）；`toml::from_str::<T>(s)` ≡ `toml::parse::<T>(s)`（round-trip 对齐紧凑输出；复用 `json_escape`/`json_unescape`）。MVP 签名降级：无泛型 protocol 约束、数组 `[T; N]` 反序列化不支持（用 Vec）、f64 不支持、紧凑输出（标准 TOML 空格形式/`[section]`/注释/多行字符串规划中）。
