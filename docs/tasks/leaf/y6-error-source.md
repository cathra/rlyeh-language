...
- **实施（io/error.rl + future.rl）**：
  - `trait Error { fn message(&self) -> String; fn source(&self) -> Option<&dyn Error>; }`（恢复目标签名）。
  - `impl Error for IoError` + `impl Error for TimeoutError` 的 `source()` 返回 `Option::None`（底层错误无源）。
- **typechecker 配套修复（发现并修复两处语言级缺陷）**：
  1. **自引用 trait 解析（resolve.rs + collect.cs + context.rs）**：自引用 trait（如 `trait Error { fn source(&self) -> Option<&dyn Error> }`）在收集自身方法签名时需引用尚未注册进 `trait_defs` 的自身名字。`collect_trait` 收集期间设置 `TypeContext::collecting_trait`（结束恢复），`resolve.rs` 的 `dyn` 分支在 `resolve_trait_key` 未命中且当前正收集同名 trait 时回退到自身（裸名/模块前缀均可），使 `&dyn Error` 在 trait 定义完成前可解析。
  2. **`collect_impl` trait 类型实参解析顺序（collect.rs）**：`collect_impl` 此前在**恢复 `ctx.type_params` 之后**才解析 `trait_type_args`，导致 `impl<T> Wrap<T> for Pair<T>` 的 `Wrap<T>` 类型实参 `T`（impl 级泛型）在 `type_params` 已清空时解析报 `undefined type T`（预存缺陷，此前被 std `Error` 自引用收集失败「抢先报错」掩盖，全局测试长期假性失败）。修复：将 `trait_type_args` 的解析移到恢复 `type_params` 之前（此时 `ctx.type_params` 仍为 `imp.generics`）。
...
| 2026-08-29 | P7d-1 `source()` 升级为 `Option<&dyn Error>` 真实错误链（Y6c）：io/error.rl + future.rl 恢复目标签名；typechecker 修复自引用 trait 解析（`collecting_trait`）+ `collect_impl` `trait_type_args` 解析顺序（暴露并修复预存 `undefined type T` 缺陷）；`error_source.{rl,out}` 验证真实错误链（131 run-pass + 12 compile-pass 全绿） |
