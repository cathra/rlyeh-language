...
> **级别**：P2（集成建设） · **风险**：🔴 高 · **状态**：🟡 进行中（M-M1 已落地、M-M2a 已落地、M-M2b 已落地（b1 语句骨架 + b2 模式/类型标注 + b3 元组/数组类型标注））、M-M2c 已落地（控制流 if/else/else-if/while/loop 的语句/块尾/let 初始化双形态）、M-M2d 已落地（字段访问 .field，shunting-yard 后缀标记 + 归约，无递归）） · **归属**：0.2.0-M
...
- **M-M2（中）** 用 Rlyeh 重写 `parser`（依赖 N/O/P + 递归下降），对拍 AST 一致。
  ...
  - **M-M2d（✅ 已落地，2026-09-22）** 表达式补全·字段访问 `.field`：Rlyeh 版 `self-host/parser.rl` 在 M-M2a 的 shunting-yard `parse_expr` 中新增 `.name` 后缀处理——推入 `@field@NAME` 标记入 RPN，在末位 RPN 归约阶段（`tk.get(0)==64` 分支）弹出栈顶节点包裹为 `(field <RECV> <NAME>)`，支持链式 `a.b.c`、分组 `(a+b).c`、与二元运算结合 `a.x + b.y` / `x.y == z.w`，且**不引入递归 / 新函数 / 嵌套循环**（规避 Rlyeh 前向递归与 if-表达式返回值限制）。Rust oracle 同步在 `render_expr_canonical` 新增 `FieldAccess` 分支（`(field <RECV> <FIELD>)`）；`tests/self_host_parser.rs` 新增 `m_m3a_field_access_ast_matches_rust_oracle` 差分对拍一致（含 `let` 初始化与 `if` 条件中的字段访问）。索引 `[ ]` / 调用 `( )` 因需子表达式解析（递归）留待显式栈重写（M-M2e）。
