...
## 目标
支持**带泛型参数的 protocol 声明与 impl**，并允许 impl 方法返回 `Self`，解除"注释 typecheck 自身需泛型 protocol"的鸡生蛋问题，解锁前端自举。

## 技术细节
- 当前 Rlyeh 0.1.0：「protocol+impl 必须非泛型」（见 `docs/guide/13-references-limits.md`）。关联类型已在 U2 落地，本任务在其上叠加泛型参数化。
- 受影响 Rust 代码（事实依据）：`rlyeh-typecheck/src/types.rs:440` `impl Type: Protocol<Args>` 带泛型实参列表；`check_expr/generic.rs`（16.6K）、`closure.rs`（26K）依赖泛型实例化。
- 子任务（对应 0.2.0-A）：A1 泛型 protocol 声明 / A2 泛型 impl / A3 含 `Self` 返回的 impl 方法 / A4 泛型约束 `where`/`:` 收尾。

## 受影响组件
`rlyeh-typecheck`（protocol/impl 解析、泛型实例化）、前端所有需泛型 protocol 的 IR 节点。

## 缺口复核（2026-09-02，动手前）
叶子「技术细节」称「当前 Rlyeh 0.1.0：protocol+impl 必须非泛型」——**该判断已过时**。
实测四项子任务的真实状态：

| 子任务 | 规划描述 | 复核结果 |
|--------|---------|---------|
| **A1** 泛型 protocol 声明 | 待实现 | ✅ **已具备**（U8）。std 已用 `protocol From<T>` / `protocol Into<T>`（`io/error.rl`）；测试有 `protocol Wrap<T>`（`generic_ctor_protocol.rl`）、`protocol Deref<T>`（`y4b_deref.rl`）。parser `parse_protocol` 早接 `parse_generics` |
| **A2** 泛型 impl | 待实现 | ✅ **已具备（含两处限制已修复，2026-09-02 第二轮）**。std 已用 `impl<T> Iter<T>: Iterator` / `impl<T> RecvAsync<T>: Future` |
| **A3** 含 `Self` 返回 | 待实现 | ✅ **已具备**（SH-P0-3 G-M1 的 `replace_type_self` + 按值 `Self` 返回 codegen 修复）。std `error_conversion.rl` 即 `protocol From<T> { fn from(v: T) -> Self; }` |
...
覆盖：函数、impl 块内方法、protocol 抽象方法声明。
...
``type `i64` does not implement protocol `Speak` (bound on generic parameter `T`)``。
...
- `tests/run-pass/generic_impl_multi_ext.{rl,out}`（5 行输出，2026-09-02 补测）：A2
  扩展覆盖——① 同 self_type **三 impl**（i64/bool/String）按 protocol 类型实参 / 实参
  选取；② 泛型 self_type `Pair<T>` + 泛型 impl 参数，T 由**接收者**推导；③ 模糊情形
  （精确 impl `Wrap<i64>` 与泛型 impl `Wrap<T>` 并存）实参 i64 时**优选具体 protocol
  类型实参的 impl**（确定性 105 = 100+5）。同时压实「同 protocol 多 impl 单态化缓存键
  含 `protocol_type_args`」修复（三 impl 方法体各异，均被调用）。
...
1. **同一类型的同一泛型 protocol 多 impl 无法按 protocol 类型实参选择**——
   `impl W: Wrap<i64>` 与 `impl W: Wrap<bool>` 并存时，`w.wrap(true)` 会选中
   ...
   **根因（双重）**：① `ctx.find_impl_for_method` 为**首匹配即返回**，候选 impl 不参与
   ...
   `impl_def.type_params`（此处为空），**未含 `protocol_type_args`**，导致两个 impl 的
   mono 键完全相同 → `mono_instances` 缓存命中复用首个实例，方法体 / 接收者错配
   （表现为跨调用结果异常：`w1.wrap(5)` 在 `w1.wrap(true)` 之后返回 5 而非 8）。
   **修复**：`check_method_call` 改为收集全部候选 impl，按「代入 protocol 类型实参后
   的方法签名与实参类型兼容」选取首个匹配者；`instantiate_impl_method` 的 mono 键 /
   后缀并入 `protocol_type_args`（经 subst 代入后）。
2. **impl 的类型参数只能由接收者类型 unify 推导**，protocol 类型实参不参与绑定——
   故 `impl<T> W: Wrap<T>`（`W` 非泛型，且 `T` 仅出现在 protocol 实参位置）报
   `undefined type T`；`impl<T> P: Wrap<T>air<T>` 则正常（`T` 由 `Pair<T>` 绑定）。
   **修复**：选取候选时，将 impl / 方法级未定泛型参数（`cand.type_params` ∪ 方法
   泛型）由对应实参类型 `unify` 反推绑定，使 `impl<T> W: Wrap<T>` 的 `T` 可由
   实参推导。
...
3. **`where` 泛型 impl 与精确 impl 并存时未回退**（A2+A4 交集，2026-09-02 补测发现）：
   ...
   `type i64 does not implement protocol Num`，而未回退到完全匹配的精确 impl（Rust 会
   选精确 impl 返回 105）。**根因**：候选选择按「代入 protocol 类型实参后的方法签名与
...
A2 两处限制（多 impl 按实参选择 + impl 泛型由实参推导 + 同 protocol 多 impl 单态化
...
| 2026-09-02 | **A2 两处限制修复（第二轮）**：① 方法解析改为收集全部候选 impl，按「代入 protocol 类型实参后的方法签名与实参兼容」选取首个匹配者（`check_method_call` 重构 + `find_impl_candidates` / `find_protocol_method_candidates`）；② impl / 方法级泛型参数由对应实参反推绑定；③ `instantiate_impl_method` 的 mono 键 / 后缀并入 `protocol_type_args`，修复同 protocol 多 impl 单态化缓存碰撞（此前 `w1.wrap(5)` 在 `w1.wrap(true)` 之后返回 5 而非 8）。新增 `generic_impl_multi.rl`（5 行输出）+ `generic-impl-mismatch.rl` 门禁，全量套件通过 |
