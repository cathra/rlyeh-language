...
|------|------|---------|:---:|------|------|
| **M** | 错误处理基底 | `IoErrorKind`/`IoError`（M1a/M1b）、`Error`/`From`/`Into` trait（M2a/M2b）、std 错误约定 Result 化（M3a/M3b/M3c） | 7 | K1、H4、G1 | ✅ 已完成 |
...
| **Q** | 序列化与格式化 trait | serde trait/derive（Q1a/Q1b/Q1c）、json 泛型 API（Q2a/Q2b）、`Display`/`Debug` + Formatter（Q3a/Q3b）、TOML（Q4） | 8 | I、L2、H4 | ✅ 已完成（Q1–Q4） |
...
| M2a `Error` trait | [`m2a-error-trait.md`](leaf/m2a-error-trait.md) | ✅ 已完成 |
...
| 子任务 | 叶子文档 | 状态 |
|--------|---------|------|
| Q1a `Serialize`/`Deserialize` trait 定义 | [`q1a-serde-trait.md`](leaf/q1a-serde-trait.md) | ✅ 已完成 |
...
| Q3a `Display`/`Debug` trait + `Formatter` | [`q3a-display-debug.md`](leaf/q3a-display-debug.md) | ✅ 已完成 |
...
| S1a `Future`/`Poll` trait 定义 | [`s1a-future-poll.md`](leaf/s1a-future-poll.md) | ✅ 已完成 |
...
| T2 `Iterator` trait 定义 | [`t2-iterator-trait.md`](leaf/t2-iterator-trait.md) | ✅ 已完成 |
