...
| **V** | 集合与迭代器完整化 | V1 借用迭代器、V2 String 码点迭代器、V3 默认方法 + 适配器迁移、V4 `get_mut`、V5 新集合、V5b 集合运算、V5c 借用迭代器、V5d 运算符重载 + 集合运算符糖 | 8 | U1/U2/U3 | ✅ 已完成（V1/V2/V3/V4/V5/V5b/V5c/V5d 全部完成；V3 自定义迭代器适配器已迁移为 trait 默认方法 + 包装迭代器，数组/`Vec` 适配器因数组非命名类型保留内建 desugar，记为已知语言限制；V5d 运算符重载为通用语言机制，仅 BinaryOp 可重载，比较链 `<`/`>` 子集/超集语义仍走命名方法） |
...
| V5d 运算符重载（语言机制）+ 集合运算符糖 | [`v5d-operator-overload.md`](leaf/v5d-operator-overload.md) | ✅ 已完成 |
