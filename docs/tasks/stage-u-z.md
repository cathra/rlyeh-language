...
| **V** | 集合与迭代器完整化 | V1 借用迭代器、V2 String 码点迭代器、V3 默认方法 + 适配器迁移、V4 `get_mut`、V5 新集合 | 5 | U1/U2/U3 | 🔧 进行中 → **见 [V2](./v2-str-view.md) / [V3](./v3-iterator-adapters.md) 任务文档**（V1/V4/V5 ✅；V3 默认方法 ✅；V2 打印/参数 ✅） |
...
| **X** | 序列化/格式化/时间完整化 | X1 Duration/Instant/SystemTime、X2 标准 TOML、X3 Deserialize + Serializer、X4 Formatter | 4 | U3/U4、Q | 🔧 进行中（X1 ✅；X2/X3/X4 规划） |
...
|---------|-----------------|---------------------|------|
| V2（`&str`） | [`v2-str-view.md`](./v2-str-view.md) | `leaf/v2-a-semantics.md` … `leaf/v2-e-api-align.md` | 🔧（V2-C/V2-D ✅） |
| V3（Iterator） | [`v3-iterator-adapters.md`](./v3-iterator-adapters.md) | `leaf/v3-a1..a4.md`（关联类型，低/中）+ `leaf/v3-b/c.md` + `leaf/v3-d1..d5.md`（适配器，中/低）+ `leaf/v3-e-builtin-cleanup.md` | 📋（默认方法基础 ✅；V3-A/V3-D 已分解为低中风险子任务） |
...
| 子任务 | 叶子文档 | 状态 |
|--------|---------|------|
| V1 借用迭代器瘦指针 MVP | [`v1-borrow-iter.md`](leaf/v1-borrow-iter.md) | ✅ 已完成 |
...
| V2-B：`&str` 子区间视图（trim/trim_start/trim_end） | [`v2-b-substring.md`](leaf/v2-b-substring.md) | 📋 规划 |
| V2-C：`&str` 打印链路修复（含 by_value StrFat 运行时） | [`v2-c-print.md`](leaf/v2-c-print.md) | ✅ 已完成（方案 A，2026-08-26） |
| V2-D：`&str` 参数/返回值完整支持 | [`v2-d-params.md`](leaf/v2-d-params.md) | 🔧 部分完成（`&str` 函数参数 ✅，返回值/深拷贝待补） |
| V2-E：String API 对齐 + 兼容保留 | [`v2-e-api-align.md`](leaf/v2-e-api-align.md) | 📋 规划 |
| V3-A1：parser — trait 内关联类型声明载体 | [`v3-a1-trait-type-parser.md`](leaf/v3-a1-trait-type-parser.md) | 📋 规划 |
| V3-A2：typecheck — 关联类型定义 + `Self::Item` 投影 | [`v3-a2-assoc-type-resolve.md`](leaf/v3-a2-assoc-type-resolve.md) | 📋 规划 |
| V3-A3：std — `Iterator::Item` 落地 + 各 impl 具体化 | [`v3-a3-iterator-item.md`](leaf/v3-a3-iterator-item.md) | 📋 规划 |
| V3-A4：`F::Item` 关联类型投影完善 + 自定义迭代器用例 | [`v3-a4-item-projection.md`](leaf/v3-a4-item-projection.md) | 📋 规划 |
| V3-B：剩余默认方法 `chain`/`enumerate`/`find`/`fold` | [`v3-b-default-methods.md`](leaf/v3-b-default-methods.md) | 📋 规划 |
| V3-C：包装迭代器类型 | [`v3-c-wrapper-iterators.md`](leaf/v3-c-wrapper-iterators.md) | 📋 规划 |
| V3-D1：`map`/`filter` trait 默认方法 + 绑定包装迭代器 | [`v3-d1-map-filter-methods.md`](leaf/v3-d1-map-filter-methods.md) | 📋 规划 |
| V3-D2：`take`/`skip` trait 默认方法 + 绑定包装迭代器 | [`v3-d2-take-skip-methods.md`](leaf/v3-d2-take-skip-methods.md) | 📋 规划 |
| V3-D3：`collect` trait 默认方法 + `fold` 归约迁移 | [`v3-d3-collect-fold-methods.md`](leaf/v3-d3-collect-fold-methods.md) | 📋 规划 |
| V3-D4：typecheck `try_check_adapter` 迁移为通用 trait 方法解析 | [`v3-d4-adapter-dispatch.md`](leaf/v3-d4-adapter-dispatch.md) | 📋 规划 |
| V3-D5：删除内建 desugar 特判 + 全量回归 | [`v3-d5-builtin-cleanup.md`](leaf/v3-d5-builtin-cleanup.md) | 📋 规划 |
| V3-E：内建 desugar 清理 + 全量回归 | [`v3-e-builtin-cleanup.md`](leaf/v3-e-builtin-cleanup.md) | 📋 规划 |
...
| X2 标准 TOML + 解析鲁棒性 | [`x2-standard-toml.md`](leaf/x2-standard-toml.md) | 📋 规划 |
| X3 `Deserialize` trait + Serializer/Deserializer 框架 | [`x3-deserialize-trait.md`](leaf/x3-deserialize-trait.md) | 📋 规划 |
