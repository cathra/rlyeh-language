...
|------|------|--------|:---:|------|------|
| **U** | 编译器地基 | U1 作用域栈、U2 关联类型、U3 泛型约束、U4 `-> Self`、U5 AddrOf、U6 Cast IR、U7 方法级泛型、U8 泛型结构体 + 泛型 protocol | 8 | G、T | ✅ 全部完成（U1–U6 + U7/U8） |
| **V** | 集合与迭代器完整化 | V1 借用迭代器、V2 String 码点迭代器、V3 默认方法 + 适配器迁移、V4 `get_mut`、V5 新集合、V5b 集合运算、V5c 借用迭代器、V5d 运算符重载 + 集合运算符糖 | 8 | U1/U2/U3 | ✅ 已完成（V1/V2/V3/V4/V5/V5b/V5c/V5d 全部完成；V3 自定义迭代器适配器已迁移为 protocol 默认方法 + 包装迭代器，数组/`Vec` 适配器因数组非命名类型保留内建 desugar，记为已知语言限制；V5d 运算符重载为通用语言机制（BinaryOp 可重载 + 比较链 `Lt`/`Le`/`Gt`/`Ge` 经 `PartialOrd` 重载子集/超集语义，见 v5d1）） |
...
| **X** | 序列化/格式化/时间完整化 | X1 Duration/Instant/SystemTime、X2 标准 TOML、X3 Deserialize + Serializer、X4 Formatter | 4 | U3/U4、Q | ✅ 已完成（X1/X2/X3/X4 全部完成：X3 `json::try_parse`/`toml::try_parse` Err 路径 + `Deserialize` protocol + Serializer/Deserializer 框架全落地，2026-08-30） |
...
| V3（Iterator） | [`v3-iterator-adapters.md`](./v3-iterator-adapters.md) | `leaf/v3-a1..a4.md`（关联类型，低/中）+ `leaf/v3-b/c.md` + `leaf/v3-d1..d5.md`（适配器，中/低）+ `leaf/v3-e-builtin-cleanup.md` | ✅（A~E 子任务已落地；自定义迭代器适配器迁移为 protocol 默认方法 + 包装迭代器惰性化；数组/Vec 适配器因数组非命名类型保留内建 desugar，记为语言限制） |
...
| U2 protocol 关联类型 | [`u2-assoc-type.md`](leaf/u2-assoc-type.md) | ✅ 已完成 |
| U3 泛型 protocol 约束（bound / where） | [`u3-generic-bound.md`](leaf/u3-generic-bound.md) | ✅ 已完成 |
...
| U8 泛型结构体构造 + 泛型 protocol | [`u8-generic-ctor-protocol.md`](leaf/u8-generic-ctor-protocol.md) | ✅ 已完成 |
...
| V3-A1：parser — protocol 内关联类型声明载体 | [`v3-a1-protocol-type-parser.md`](leaf/v3-a1-protocol-type-parser.md) | ✅ 已完成（含默认具体化，2026-08-27） |
| V3-A2：typecheck — 关联类型定义 + `Self::Item` 投影 | [`v3-a2-assoc-type-resolve.md`](leaf/v3-a2-assoc-type-resolve.md) | ✅ 已完成（U2 核实，2026-08-27） |
| V3-A3：std — `Iterator::Item` 落地 + 各 impl 具体化 | [`v3-a3-iterator-item.md`](leaf/v3-a3-iterator-item.md) | ✅ 已完成（protocol 签名 + 各 impl 补 type Item，2026-08-27） |
...
| V3-D1：`map`/`filter` protocol 默认方法 + 绑定包装迭代器 | [`v3-d1-map-filter-methods.md`](leaf/v3-d1-map-filter-methods.md) | ✅ 已完成（filter/take/skip/collect + 语言增强，2026-08-27） |
| V3-D2：`take`/`skip` protocol 默认方法 + 绑定包装迭代器 | [`v3-d2-take-skip-methods.md`](leaf/v3-d2-take-skip-methods.md) | ✅ 已完成（map + Iter/IterMut 实现 Iterator，2026-08-27） |
| V3-D3：`collect` protocol 默认方法 + `fold` 归约迁移 | [`v3-d3-collect-fold-methods.md`](leaf/v3-d3-collect-fold-methods.md) | ✅ 已完成（fold 惰性化，2026-08-27） |
| V3-D4：typecheck `try_check_adapter` 迁移为通用 protocol 方法解析 | [`v3-d4-adapter-dispatch.md`](leaf/v3-d4-adapter-dispatch.md) | ✅ 已完成（双轨收敛，2026-08-27） |
...
| X3 `Deserialize` protocol + Serializer/Deserializer 框架 | [`x3-deserialize-protocol.md`](leaf/x3-deserialize-protocol.md) | ✅ 已完成（`Deserialize` protocol + `JsonError`/`TomlError` + `Serializer`/`Deserializer` 访问器框架 ✅；`json::try_parse`/`toml::try_parse` Err 路径 ✅，2026-08-30） |
