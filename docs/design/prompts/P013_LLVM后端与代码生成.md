...
> **目标**：打通编译流水线最后两环 —— LIR 三地址码 → LLVM IR 文本 → clang 汇编链接 → 可执行文件，使 `rlyeh run hello-world.rl` 端到端可用。
...
| crate | 职责 |
|-------|------|
| `rlyeh-lir` | 低级中间表示（三地址码）+ MIR → LIR lowering |
| `rlyeh-codegen` | LIR → LLVM IR 文本生成 |
| `rlyeh-driver` | 编译器驱动（CLI）：串联全流水线 + 调用 clang 汇编/链接 |

### 1.2 验收标准

- [x] MIR 程序可降低为三地址码 LIR（含局部变量类型推断）
- [x] LIR 可生成合法 LLVM IR 文本，clang 可汇编链接成可执行文件
- [x] `rlyeh run examples/hello-world.rl` 输出 `Hello, Rlyeh!`
...
Rlyeh 的 `main` 函数生成 `define i32 @main()`，返回 `ret i32 0`。
...
- **理由**：MVP 的分配语义（bump allocator + LIFO 析构）由 `rlyeh-region-alloc` 运行时库提供，LLVM IR 生成阶段不展开；后续由专用后端或运行时接线。
...
- `build_executable(source)`：IR 落盘临时目录（`rlyeh-mvp-{pid}-{nanos}`）→ 探测 clang → `clang -O1 ir -o exe`
- `run_source(source)`：build + 执行 + 捕获输出
- clang 探测屏蔽输出（`.stdout(Stdio::null())`），避免 `clang --version` 污染 stdout

`src/main.rs`：CLI 入口 —— `rlyeh run <file>` / `rlyeh build <file>` / `--version`。

### 3.4 配套改动

- `rlyeh-typecheck/src/check_expr.rs`：新增 `pub const BUILTIN_FUNCTIONS`；Call 分支在宏分支前处理内建（保留参数、返回 `Unit`）
- `rlyeh-mir/src/passes/dce.rs`：修复 **既有 bug** —— 副作用调用（`print`/`println` 等 `Call`）被 `remove_dead_assignments` 误删；修复为 Call 永不删除、参数标记活跃
...
| 文件 | 覆盖点 | 数量 |
|------|--------|------|
| `rlyeh-lir/tests/lir_lower_test.rs` | 三地址运算、比较 CondJump、内建调用保留参数、区域标注、类型推断、嵌套拆平 | 6 |
| `rlyeh-codegen/tests/llvm_test.rs` | 字符串/printf、用户函数、icmp+CondJump、not+select、转义、未定义函数报错 | 6 |
| `rlyeh-driver/tests/driver_test.rs` | **端到端**：clang 汇编/链接/运行，断言精确 stdout 输出 | 6 |
| `rlyeh-mir` 既有测试 | DCE 副作用调用修复回归 | — |

**结果**：全 workspace 230 个测试全部通过；`cargo clippy --workspace --all-targets` 零警告；`cargo fmt --all -- --check` 干净。

**端到端示例**（`examples/`）：

- `hello-world.rl`：`fn main() { println("Hello, Rlyeh!"); }` → 输出 `Hello, Rlyeh!`
- `arith-print.rl`：函数加法 + 内建打印 + if 比较 → 输出精确匹配
...
| `fn add(a, b)` 报 Comma 语法错误 | Rlyeh 要求参数带类型标注 | 示例改为 `fn add(a: i64, b: i64) -> i64` |
...
| 日期 | 内容 |
|------|------|
| 2026-08-19 | 搭建 `rlyeh-lir`（三地址码 + MIR→LIR lowering + 类型推断），6 个单测 |
| 2026-08-19 | 搭建 `rlyeh-codegen`（LLVM IR 文本 + 内建 printf），6 个单测 |
| 2026-08-19 | 搭建 `rlyeh-driver`（CLI + clang 端到端），6 个端到端测试；typecheck 注册内建 |
